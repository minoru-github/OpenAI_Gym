...
class Agent():
    ...
    def replay(self):
        ...
        next_states = np.array([(np.zeros(self.shape) if experience[3] is None else experience[3]) for experience in batch])
        ...
        x = np.zeros((batch_len, self.shape[0], self.shape[1]))  # x[batch_cnt][num_states]
    ...
    def act(self, state, episode):
        ...
        if random_val >= epsilon:
            action = np.argmax(self.trgt_q_net.predict(state.reshape(1,self.shape))) # best action
            print("best:",action)
        else:
